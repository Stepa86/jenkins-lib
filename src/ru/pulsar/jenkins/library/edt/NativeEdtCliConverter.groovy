...
class NativeEdtCliConverter implements IEdtCliEngine {
    ...
    void edtToDesignerTransformConfiguration(IStepExecutor steps, JobConfiguration config) {
        ...
        def edtcliCommand = "1cedtcli -data \"$projectWorkspaceDir\" -vmargs -DdisableFTS=true -command export --configuration-files \"$configurationRootFullPath\" --project \"$projectDir\""
    ...
    void edtToDesignerTransformExtensions(IStepExecutor steps, JobConfiguration config) {
        ...
        config.initInfoBaseOptions.extensions.each {
            ...
            def edtcliCommand = "1cedtcli -data \"$currentExtensionWorkspaceDir\" -vmargs -DdisableFTS=true -command export --configuration-files \"$extensionRoot/${it.name}\" --project \"$currentExtensionProjectDir\""
    ...
    void designerToEdtTransform(IStepExecutor steps, JobConfiguration config) {
        ...
        def edtcliCommand = "1cedtcli -data \"$workspaceDir\" -vmargs -DdisableFTS=true -command import --configuration-files \"$configurationRoot\" --project-name \"$projectName\""
    ...
    void edtValidate(IStepExecutor steps, JobConfiguration config, String projectList) {
        ...
        def edtcliCommand = "1cedtcli -data \"$workspaceLocation\" -vmargs -DdisableFTS=true -command validate --file \"$resultFile\" $projectList"
        steps.catchError {
            steps.cmd(edtcliCommand)
        }
